    ...
    await axios
      ...
      .then((resp) => {
        ...
        localStorage.setItem(
          adminInformation,
          JSON.stringify({
            admin_username: resp?.data?.student?.admin_username,
          })
        );

        Cookies.set(adminIdToken, resp.data.idToken);
        localStorage.setItem(adminJwtToken, resp.data.token);
        router.push(StudentRouteEnum.adminDashboard);
      })
      .catch((err) => {
        console.log("");
        setError(true);
        ...
        <div className=" flex h-full grow items-center justify-center border-white/20 py-10 text-center dark:border-l md:py-0">
          <div className="mx-auto flex w-full max-w-[450px] flex-col items-center justify-center px-5">
            <h1 className="font-inter mb-10 text-3xl font-semibold tracking-wider">
              {props.mode === "admin" ? "Admin Log In" : "Admin Log In"}
            </h1>
            <TextField
              title={props.mode === "admin" ? "ID" : "Admin ID"}
              placeholder="ID"
              type="text"
